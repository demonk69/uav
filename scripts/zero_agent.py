#!/usr/bin/env python3
"""Run the M1 environment with zero actions for a finite number of steps."""
...
parser = argparse.ArgumentParser(description="Zero-action smoke agent for the UAV rendezvous M1 task.")
...
def main() -> None:
    ...
    try:
        ...
        print(f"[INFO] zero_agent completed {args_cli.steps} steps for {env.unwrapped.num_envs} environments.", flush=True)
    finally:
        if env is not None:
            env.close()
